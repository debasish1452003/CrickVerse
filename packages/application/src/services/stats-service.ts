...
import { StandingsCalculator, type StandingRow } from "@crickverse/domain";
import { Venue } from "@crickverse/domain";
...
export class StatsService {
  ...
  async tournamentStats(
    ...
    const photos = await this.stats.playerPhotosByIds(playerIds);
    ...
    return {
      mostRuns: top(batArr, (a, b) => b.runs - a.runs, limit).map((p) => ({
        ...
        name: p.name,
        photoUrl: p.cricsheetId ? photos.get(p.cricsheetId) ?? null : null,
      ...
      highestScores: top(batting, (a, b) => b.runs - a.runs, limit).map((b) => ({
        ...
        name: b.name,
        photoUrl: b.cricsheetId ? photos.get(b.cricsheetId) ?? null : null,
        value: `${b.runs}${b.out ? "" : "*"}`,

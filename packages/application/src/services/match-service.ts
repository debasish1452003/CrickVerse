...
import { CanonicalMatch } from "@crickverse/domain";
import { GoldMatch } from "@crickverse/domain";
import { MatchMapper, type GoldMatchListItem, type InningsOversData, type MatchDTO } from "../dto/match-dto";
import type { MatchFilter, MatchRepository } from "../ports/match-repository";
...
export class MatchService {
  ...
  /** Paginated matches for a team id (canonical home/away membership), newest first. */
  async forTeam(
    teamId: string,
    page = 1,
    ...
    const total = await this.matches.countTeamMatches(teamId, gender);
    const { page: p, pageCount } = resolvePage(page, total, pageSize);
    const items = await this.matches.pageTeamMatches(teamId, (p - 1) * pageSize, pageSize, gender);
